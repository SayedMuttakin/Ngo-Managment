...
router.post('/login', validateLogin, async (req, res) => {
  try {
    ...
    console.log('📊 Database state:', require('mongoose').connection.readyState); // 0: disconnected, 1: connected
    ...
    if (!isSuperAdmin) {
      console.log('⏰ Checking login time restrictions...');
      const settings = await SystemSettings.getSettings();
      console.log('⚙️ System settings retrieved:', settings ? 'Success' : 'Failed');
      ...
      if (!loginCheck.allowed) {
        ...
        console.log('🚫 Login time restricted:', loginCheck);
